...
import { Box, Title, Text, Stack, Paper, Group, ActionIcon, Button, Overlay, Badge, useMantineColorScheme } from '@mantine/core';
  ...
  timer: [
    {
      ...
      description: 'The Abide Guide features a convenient countdown timer located in the top navigation bar. This timer can help you manage your study sessions, quiet times, or any activity where you want to track time.',
    },
    {
      id: 'timer-display',
      title: 'Timer Display',
...
export function WalkthroughPanel({ walkthroughType, onClose }: WalkthroughPanelProps) {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      document.querySelectorAll('[data-walkthrough], [data-timer-display], [data-timer-start-button], [data-timer-stop-button]').forEach((el) => {
    ...
    >
      ...
      <Box style={{ flex: 1, overflowY: 'auto', padding: '16px' }}>
        <Paper p="md" withBorder ref={stepRef}>
          <Stack gap="md">
            <Box>
              <Title order={3} mb="sm" size="h4">{currentStepData.title}</Title>
              <Text size="sm" c="dimmed">{currentStepData.description}</Text>
            </Box>

            {currentStepData.id === 'overview' && (
              <Box>
                <Text size="sm" mb="md">Follow along as we explore each step. Use the navigation buttons below to move through the walkthrough.</Text>
              </Box>
            )}

            {currentStepData.id === 'complete' && (
              <Box>
                <Text size="sm" fw={500} mb="md">You're ready to get started!</Text>
              </Box>
            )}
          </Stack>
        </Paper>
